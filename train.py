...
def train(opt, logger):
    ...
    for epoch in range(start_epoch, opt.epochs):
        for data_i in datasets.target_train_loader:
            ...
            elif opt.stage == 'baseline':
                loss_GTA = model.baseline(images, labels, target_image, target_imageS, target_params, target_lp,
                                        target_lpsoft, target_image_full, target_weak_params)
            elif opt.stage == 'stage1':
                loss, loss_CTS, loss_GTA, queue = model.step(images, labels, target_image, queue, queue_label, use_the_queue, target_imageS, target_params, target_lp,
                                        target_lpsoft, target_image_full, target_weak_params)
            else:
            ...
            if (i + 1) % opt.print_interval == 0:
                if opt.stage == 'warm_up':
                    ...
                    fmt_str = "Epochs [{:d}/{:d}] Iter [{:d}/{:d}]  loss_GTA: {:.4f} Time/Image: {:.4f}"
                    print_str = fmt_str.format(epoch+1, opt.epochs, i + 1, opt.train_iters, loss_GTA, time_meter.avg / opt.bs)
                elif opt.stage == 'stage1':
                    fmt_str = "Epochs [{:d}/{:d}] Iter [{:d}/{:d}]  loss: {:.4f}  loss_CTS: {:.4f}  loss_GTA: {:.4f} Time/Image: {:.4f}"
                    print_str = fmt_str.format(epoch+1, opt.epochs, i + 1, opt.train_iters, loss, loss_CTS, loss_GTA, time_meter.avg / opt.bs)
                else:
