...
class CustomModel():
    def __init__(self, opt, logger, isTrain=True):
        ...
        if self.opt.finetune or self.opt.stage == 'warm_up' or self.opt.stage == 'baseline':
    ...
    def baseline(self, source_x, source_label, target_x, target_imageS=None, target_params=None, target_lp=None, 
            target_lpsoft=None, target_image_full=None, target_weak_params=None):

        assert target_lpsoft is not None
        ...
        labels = threshold_arg.max(1, keepdim=True)[1]

        argmax = threshold_arg.max(1, keepdim=True)[0]
        labels[argmax < self.opt.train_thred] = 250

        loss_GTA = cross_entropy2d(input=target_out['out'], target=labels.reshape([batch, w, h]))
    ...
    def calculate_queue_vector(self, feat_cls, outputs, labels=None, thresh=None): #feat [l, 256] label [l, 1]
        h = 256
        w = int(self.opt.queue_length / h)
        feat_cls = feat_cls.transpose(1, 0).contiguous().view(256, h, w).unsqueeze(0) #bs, 256, h, w
        outputs = outputs.transpose(1, 0).contiguous().view(1, h, w).unsqueeze(0) #bs, 1, h, w
        outputs_softmax = outputs
        if thresh is None:
            thresh = -1
        mask = outputs_softmax.ge(thresh)
        outputs_argmax = self.process_label(outputs_softmax.float())
        if labels is None:
            outputs_pred = outputs_argmax
        else:
            labels_expanded = self.process_label(labels)
    ...
    def step(self, source_x, source_label, target_x,  queue, queue_label, use_the_queue, target_imageS=None, target_params=None, target_lp=None, 
            target_lpsoft=None, target_image_full=None, target_weak_params=None):
        ...
        batch, cls_n, w, h = threshold_arg.shape
        if self.opt.proto_rectify:
            weights, feat_short = self.get_prototype_weight(ema_out['feat'], target_weak_params=target_weak_params)
            rectified = weights * threshold_arg
            rectified_ori = rectified
        ...
        #short-term memory
        prob_short = self.full2weak(ema_out['out'], target_weak_params=target_weak_params)
        threshold_prob_SL = F.softmax(prob_short, dim=1) # bs,cls,h,w
        #rectify label for short-term memory
        rectified_SL = weights * threshold_prob_SL
        threshold_SL_arg = rectified_SL.max(1, keepdim=True)[1]
        rectified_SL = rectified_SL / rectified_SL.sum(1, keepdim=True)
        argmax_SL = rectified_SL.max(1, keepdim=True)[0]
        threshold_SL_arg[argmax_SL < self.opt.train_thred] = 250
        #resize
        feat_SL = feat_short.transpose(1, 2).transpose(2, 3).contiguous().view(self.opt.bs, -1, 256)
        threshold_SL = F.interpolate(threshold_SL_arg.float(), size=(feat_short.shape[2], feat_short.shape[3])).long()
        label_SL_DS = threshold_SL.contiguous().view(self.opt.bs, -1, 1)
        threshold_prob_SL = threshold_prob_SL.transpose(1, 2).transpose(2, 3).contiguous().view(self.opt.bs, -1, cls_n)


        #update short-term memory
        for i in range(feat_SL.shape[0]):
            queue[self.opt.fs:] = queue[:-self.opt.fs].clone()
            # select feature
            entropy = self.compute_entropy(threshold_prob_SL[i])
            _, top_indices = torch.topk(entropy, self.opt.fs)
            queue[:self.opt.fs] = feat_SL[i][top_indices]
            queue_label[self.opt.fs:] = queue_label[:-self.opt.fs].clone()
            queue_label[:self.opt.fs] = label_SL_DS[i][top_indices]


        #use short-term memory
        if not torch.all(queue_label[-1, :]==0):
            use_the_queue = True
            short_vectors, short_ids = self.calculate_queue_vector(queue.detach(), queue_label.detach())

            # denoise with short-term memory
            short_proto = torch.zeros_like(self.objective_vectors)
            for i in range(self.class_numbers):
                if i in short_ids:
                    t_s = short_ids.index(i)
                    short_proto[i] = short_vectors[t_s].squeeze()
                else:
                    short_proto[i] = self.objective_vectors[i]
            #denoise
            short_weights = self.get_short_weight(feat_short, short_proto)
            rectified_short = short_weights * threshold_ori
            rectified_all = 0.5 * rectified_ori + 0.5 * rectified_short
            threshold_all = rectified_all.max(1, keepdim=True)[1]
            rectified_all = rectified_all / rectified_all.sum(1, keepdim=True)
            argmax_all = rectified_all.max(1, keepdim=True)[0]
            threshold_all[argmax_all < self.opt.train_thred] = 250

            loss_CTS = cross_entropy2d(input=target_out['out'], target=threshold_all.reshape([batch, w, h]))
        else:
        ...
        if self.opt.moving_prototype: #update prototype
            ...
            for t in range(len(ema_ids)):
                if use_the_queue and (ema_ids[t] in short_ids):
                    t_s = short_ids.index(ema_ids[t])
                    ema_vectors_ = (1 - self.opt.proto_momentum) * ema_vectors[t] + self.opt.proto_momentum * short_vectors[t_s]
                    self.update_objective_SingleVector(ema_ids[t], ema_vectors_.detach(), start_mean=False)
                else:
        ...
        return loss.item(), loss_CTS.item(), loss_GTA.item(), queue
    ...
    def compute_entropy(self, softmax_probs):
        
        eps = 1e-10
        entropies = -torch.sum(softmax_probs * torch.log(softmax_probs + eps), dim=1)
    ...
    def feat_short_prototype_distance(self, feat, proto):
        N, C, H, W = feat.shape
        feat_proto_distance = -torch.ones((N, self.class_numbers, H, W)).to(feat.device)
        for i in range(self.class_numbers):
            feat_proto_distance[:, i, :, :] = torch.norm(proto[i].reshape(-1,1,1).expand(-1, H, W) - feat, 2, dim=1,)
        return feat_proto_distance

    def get_short_weight(self, feat, proto):
        feat_short_proto_distance = self.feat_short_prototype_distance(feat, proto)
        feat_short_nearest_proto_distance, feat_nearest_proto = feat_short_proto_distance.min(dim=1, keepdim=True)

        feat_short_proto_distance = feat_short_proto_distance - feat_short_nearest_proto_distance
        weight = F.softmax(-feat_short_proto_distance * self.opt.proto_temperature, dim=1)
