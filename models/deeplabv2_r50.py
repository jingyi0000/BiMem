...
def Deeplab(BatchNorm, num_classes=21, freeze_bn=False, restore_from=None, initialization=None, bn_clr=False):
    model = ResNet50(Bottleneck, [3, 4, 6, 3], num_classes, BatchNorm, bn_clr=bn_clr)
    if freeze_bn:
        model.apply(freeze_bn_func)
    if initialization is None:
        pretrain_dict = model_zoo.load_url('https://download.pytorch.org/models/resnet50-11ad3fa6.pth') #https://download.pytorch.org/models/resnet50-19c8e357.pth'
    else:
    ...
    if restore_from is not None: 
        ...
        model.load_state_dict(checkpoint['ResNet50']["model_state"])
